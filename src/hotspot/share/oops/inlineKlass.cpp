...
address InlineKlass::calculate_fixed_block_address() {
  // The fix block is placed after all other fields inhereted from the InstanceKlass
...
void InlineKlass::init_fixed_block() {
  assert(_adr_inlineklass_fixed_block != 0, "Should have been initialized in constructor");
  assert(_adr_inlineklass_fixed_block == &fixed_block(), "Aha");

  // TODO: It is unclear why this isn't done in the constructor

  // Addresses used for inline type calling convention
  *adr_extended_sig() = nullptr;
  *adr_return_regs() = nullptr;
  *adr_pack_handler() = nullptr;
  *adr_pack_handler_jobject() = nullptr;
  *adr_unpack_handler() = nullptr;
    ...
    *adr_extended_sig() = extended_sig;
    ...
    if (can_be_returned_as_fields(/* init= */ true)) {
      ...
      if (total > 0) {
        ...
        *adr_pack_handler() = buffered_blob->pack_fields();
        *adr_pack_handler_jobject() = buffered_blob->pack_fields_jobject();
        *adr_unpack_handler() = buffered_blob->unpack_fields();
